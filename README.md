...
daikin-cli is a fork of [redgoose/daikin-one](https://github.com/redgoose/daikin-one), a lightweight CLI wrapper for the [Daikin Skyport API](https://github.com/redgoose/daikin-skyport) to manage Daikin One+ devices/thermostats.
...
5. With a device id, you can start managing your device. For example, to retrieve device configuration and state values use:
	...
	```
	daikin-cli device info --device-id <device-id>
	```
...
Run `daikin-cli -h` and `daikin-cli device -h` for a full list of available commands.
...
* Ensure sufficient data has been logged via the `daikin-cli log` command.
...
```
daikin-cli report summary -d <device-id>
```

To output the report to a file instead, use:

```
daikin-cli report summary -d <device-id> > report_summary.html
```
...
```
daikin-cli report day 2023-08-19 -d <device-id> > report_day.html
daikin-cli report month 2023-08 -d <device-id> > report_month.html
daikin-cli report year 2023 -d <device-id> > report_year.html
```
...
MIT © redgoose (original work), fork modifications © Omar Shahine. See [LICENSE](LICENSE) for details.
